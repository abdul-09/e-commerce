...
@pytest.mark.parametrize(
    "user_name, email, password, password2, expected_errors",
    [
        ("Abdul", "abdul@work.com", "qwerty", "qwerty", {}),
        ("Abdul", "abdul@work.com", "qwerty", "qwert", {"email": ["Email already exists."]}),
        ("Abdul", "", "qwerty", "qwert", {"email": ["This field is required."]}),
    ],
)
@pytest.mark.django_db
def test_create_account_view(client, user_name, email, password, password2, expected_errors):
    response = client.post(
        "/account/register/",
        data={
            "user_name": user_name,
    ...
    # Check if the response status code is 200 (success) if there are no form errors
